...
class DevAssetInline(admin.TabularInline):
    model = DevAsset
    extra = 0
...
class DevProjectAdmin(admin.ModelAdmin):
    list_display  = ["title", "user", "slug", "is_published", "created_at", "updated_at"]
    list_filter   = ["is_published"]
    search_fields = ["title", "slug", "user__email", "user__first_name", "user__last_name"]
    raw_id_fields = ["user"]
    readonly_fields = ["created_at", "updated_at"]
    inlines = [DevAssetInline]


@admin.register(DevAsset)
class DevAssetAdmin(admin.ModelAdmin):
    list_display  = ["project", "original_filename", "uploaded_at"]
    search_fields = ["project__title", "original_filename"]
